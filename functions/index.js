...
const CAPABILITY_ASSESSMENT_PATTERN = /\b(?:James|he)\b[^.!?]{0,160}(?:\b(?:can|could|would|may|might|appears?|seems?|looks?)\b[^.!?]{0,100}\b(?:handle|perform|work|succeed|contribute|manage|fit|suited|qualified|capable)\b|\bis\s+(?:well\s+)?(?:capable|qualified|suited)\s+(?:of|for|to)\b|\bshould\s+be\s+able\s+to\b)/i;
